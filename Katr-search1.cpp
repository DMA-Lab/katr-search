	...
	//	cout<<ru<<" "<<SCmin<<"!!!!"<<Dub<<endl<<((1-alpha)*(ru+30000)-SCmin)/alpha<<"??"<<now_Poi[0].count(1107)<<"!!!!!!!!"<<endl;
		...
		rep(i,0,m-1){
			while(1){
				...
				if(dis2[(*Pnow)]<=Dub||!Vis2[vq.id]){//每个元素最多被删一次 
	//				
			//		cout<<Dub<<" "<<dis2[(*Pnow)]<<"!!";
					break;
...
void get_batch_CP_route(CP_set CPSi){
		...
		Now_route.clear();
		fg=0;//cout<<tot<<endl;
		shuffle_route(0,0);//70ms!!!优先优化  已经优化 
		
	//	cout<<tott<<endl;
		if(!Now_route.size())return;++tot;
		rep(i,0,Now_route.size()-1){
			Qr.push(Now_route[i]);//50~70ms!!!优先优化   已优化 
...
void get_new_route(int id,Poi vf){
  // 结束时间	auto end = std::chrono::high_resolution_clock::now();  // 结束时间
...
void get_ans(){
	...
	int sz=min((int)all_route.size(),min(k,1));
	rep(i,0,sz-1){
		all_route[i].write();
...
void gettype(){
    ...
    while (getline(file, line)) {
		...
		if(values[2]>=7)continue; 
        // 提取第3列和最后一列数据
        if (values.size() >= 4) {
        	pp.pb(values[1]);//加入关键编号 
        	_Poi[values[1]].key=ss[values[2]];  //关键词 
        	//if(values[2]==2)totrr++;
            _Poi[values[1]].r=values.back()*1000;    // 积分 
        }
   ...
   // srand(time(0));
	...
	t.pb("1");t.pb("2");t.pb("3");//t.pb("5");
	t.pb("4");//
	 t.pb("5");t.pb("6");
	
	//t.pb("6");

	que=query(t,_Poi[13],4);
	rep(i,0,pp.size()-1){
		insert(pp[i]);
	} 
	//rep(i,0,200)insert(i);//插入符 
	que.vq=_Poi[13];
}
    // 计算这次运行的时间
...
int main(int argc, char** argv) {
	...
	get_ans();auto end = std::chrono::high_resolution_clock::now();std::chrono::duration<double, std::milli> duration = end - start;  // 这里正确声明了 duration
