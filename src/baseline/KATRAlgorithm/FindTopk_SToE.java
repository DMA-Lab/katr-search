...
//                    }
    ...
    public ArrayList<LowerBound> KATRFindTopk(Graph g, int q, int endIndex, int q_SG, int k, int[] POI_Type, ArrayList<ArrayList<Integer>> SG, ArrayList<ArrayList<Path>> List, POI[] POIList,
...
//        int q){
        ...
        path = Find_Path2(path2, POI_Type, POIList, all, q, 0, 0, a, endIndex);
        ...
        for (int i = 0; i < SG_n.size(); i++) {
            ...
            if (flag3) {
                ...
                if (min > (int) R) { //网格剪枝
                    ...
                    startTime1 = System.currentTimeMillis();
                    ArrayList<ArrayList<ArrayList<Integer>>> path7 = Find_Path2(path8, POI_Type, POIList, all, q, 1, LB, a, endIndex);
                    endTime1 = System.currentTimeMillis();
                    time11 = time11 + endTime1 - startTime1;
                    //startTime1 = System.currentTimeMillis();
                    int num33 = 0;
...
//        System.out.println("第六部分所消耗的时间为："+ time6);
    ...
    public ArrayList<ArrayList<ArrayList<Integer>>> Find_Path2(ArrayList<ArrayList<Integer>> path, int[] POI_Type, POI[] POIList, ArrayList<ArrayList<Integer>> all,
                                                               int q, int flag, double LB, double a, int endIndex) {
        ...
        for (int i = 0; i < find_Path.size(); i++) {
            for (int j = 0; j < find_Path.get(i).size(); j++) {
                find_Path.get(i).get(j).add(endIndex);
            }
        }
        return find_Path;
    }
