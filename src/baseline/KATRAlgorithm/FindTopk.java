...
public class FindTopk {
    ...
    public static int FYNum = 0;
    public static int NotFYNum = 0;
    public static int HashMapNum = 0;
    //    public static int[][] FYPath;
...
//                    }
    ...
    public ArrayList<LowerBound> KATRFindTopk(Graph g, int q, int q_SG, int k, int[] POI_Type, ArrayList<ArrayList<Integer>> SG, ArrayList<ArrayList<Path>> List, POI[] POIList,
                                              double a, ArrayList<ArrayList<BpPath>> BPList, ArrayList<ArrayList<Integer>> PointMinBP, ArrayList<ArrayList<Integer>> BvList) throws InterruptedException {
...
//        int[][] FYPath1 = new int[10000][10000];//欧式距离复用
//        FYPath = FYPath1;
...
//        int q){
        ...
        for (int i = 0; i < Top_k.size(); i++) {
            if (Smin > Top_k.get(i).score) {
                Smin = Top_k.get(i).score;
            }
        }
        double Du = ((1 - a) * w_max - Smin) / a;
        ...
        for (int i = 0; i < BvList.size(); i++) {
            for (int j = 0; j < BvList.get(i).size(); j++) {
                Bv = BvList.get(i).get(j);
        ...
        for (int i = 0; i < SG_n.size(); i++) {
            int SGq = SG_n.get(i); //记录SG的编号
            //记录SG中的POI
            SGPoi.add(new SGPoi());
            SGPoi.get(SGPoi.size() - 1).num = SGq;
            R = ((1 - a) * w_max - LB) / a;
            ...
            flag3 = true;
            for (int l = 0; l < SGPoi.get(SGPoi.size() - 1).POI.size(); l++) {//判断这个子图是否有全部的兴趣点
                if (SGPoi.get(SGPoi.size() - 1).POI.get(l).size() == 0) {
                    flag3 = false;
                    //break;
                    break;
                }
            ...
            if (flag3) {
                ...
                if (min > (int) R) { //网格剪枝
                    ...
                    // startTime1 = System.currentTimeMillis();
                    ArrayList<LowerBound> allPath4 = Find_allPath(path7, List, POIList, a, q_BP, Min_w, BPList, PointMinBP, k);//计算实际分数
                    //endTime1 = System.currentTimeMillis();
                    //time33 = time33 + endTime1 - startTime1;
                    //System.out.println("查询一个子图的时间=");
    ...
    public ArrayList<LowerBound> Find_allPath(ArrayList<ArrayList<ArrayList<Integer>>> path, ArrayList<ArrayList<Path>> List, POI[] POIList, double a,
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            for (int ii = 0; ii < path.get(i).size(); ii++) {
                ...
                String q;
                //boolean flag = false;
                //ArrayList<ArrayList<Integer>> FYPath = new ArrayList<>(); //计算欧式距离时服用
                startTime1 = System.currentTimeMillis();
                for (int j = 0; j < path.get(i).get(ii).size() - 1; j++) {
                    // flag = false;
                    q_index = path.get(i).get(ii).get(j);
                    q_index1 = path.get(i).get(ii).get(j + 1);
//                    for (int l = 0; l < FYPath.size(); l++) {
//                        if ((FYPath.get(l).get(0) == q_index && FYPath.get(l).get(1) == q_index1)){
//                            w += FYPath.get(l).get(3);
//                            flag = true;
//                            break;
//                        }
//                    }
                    ...
                    HashMapNum++;
//                    str_q_index = String.valueOf(q_index);
//                    str_q_index1 = String.valueOf(q_index1);
//                    startTime3 = System.nanoTime();
//                    if (q_index <= q_index1){
//                        q = str_q_index + str_q_index1;
//                    }else {
//                        q = str_q_index1 + str_q_index;
//                    }
//                    endTime3 = System.nanoTime();
//                    w_HashMap = FYPath.get(q);
//
//                    HashMapTime += (endTime3 - startTime3);
//                    HashMapNum++;
//                    if (w_HashMap == null){
...
//                        w1 = (int) sqrt(Math.pow(x,2)+Math.pow(y,2));
//                        w += w1;
//                        FYPath.put(q, String.valueOf(w1));
//                        NotFYNum++;
//                    }else {
//                        w += Integer.parseInt(w_HashMap);
//                        FYNum++;
//                    }
                }
                endTime1 = System.currentTimeMillis();
...
//        System.out.println("第六部分所消耗的时间为："+ time6);
    ...
    public ArrayList<ArrayList<ArrayList<Integer>>> Find_Path2(ArrayList<ArrayList<Integer>> path, int[] POI_Type, POI[] POIList, ArrayList<ArrayList<Integer>> all,
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            for (int j = 0; j < path.get(i).size(); j++) { //计算x轴的分布情况
                if (POIList[path.get(i).get(j)].x >= POIList[q].x) { //如果这个点在q的右边
                    ...
                    if (num16 >= x_max) { //这个点在最右边
                        ...
                        x_right.add(new x_ty(path.get(i).get(j), num16));
                    } else {
                        for (int k = 0; k < x_right.size(); k++) {
                            if (num16 <= x_right.get(k).num) {
                                x_right.add(k, new x_ty(path.get(i).get(j), num16));
                                break;
                            }
                        }
                    }
                    ...
                    if (num16 >= x_min) {
                        x_min = num16;
                        x_left.add(new x_ty(path.get(i).get(j), num16));
                    } else {
                        for (int k = 0; k < x_left.size(); k++) {
                            if (num16 <= x_left.get(k).num) {
                                x_left.add(k, new x_ty(path.get(i).get(j), num16));
                                break;
                            }
                        }
            ...
            for (int j = 0; j < path.get(i).size(); j++) { //计算y轴的分布情况
                if (POIList[path.get(i).get(j)].y >= POIList[q].y) { //如果这个点在q的右边
                    ...
                    if (num16 >= x_max) { //这个点在最右边
                        x_max = num16;
                        // x_right.add(path.get(i).get(j));
                        y_right.add(new x_ty(path.get(i).get(j), num16));
                    } else {
                        for (int k = 0; k < y_right.size(); k++) {
                            if (num16 <= y_right.get(k).num) {
                                y_right.add(k, new x_ty(path.get(i).get(j), num16));
                                break;
                            }
                        }
                    }
                    ...
                    if (num16 >= x_min) {
                        x_min = num16;
                        y_left.add(new x_ty(path.get(i).get(j), num16));
                    } else {
                        for (int k = 0; k < y_left.size(); k++) {
                            if (num16 <= y_left.get(k).num) {
                                y_left.add(k, new x_ty(path.get(i).get(j), num16));
                                break;
                            }
                        }
                    }
            ...
            x_left.add(0, new x_ty(q, 0));
            x_right.add(0, new x_ty(q, 0));
            y_left.add(0, new x_ty(q, 0));
            y_right.add(0, new x_ty(q, 0));
