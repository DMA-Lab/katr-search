...
public class FindTopK_SToE {
    ...
    //public int num_LJ5 = 0;//判断每次总的组合里面有多少条组合形式
    ...
    public ArrayList<ArrayList<Integer>> allsort(int[] a, ArrayList<ArrayList<Integer>> order, int currentlocal) // currentlocal当前指向的位置
    {
        ...
        if (currentlocal == a.length - 1) // 当当前位置指到最后一个元素时，该元素后面已经没有其他元素可以跟他交换位置，即已产生一个组合数
        {
            ...
            for (int i = currentlocal; i < a.length; i++) {
                ...
                // 交换两个元素的位置后，对其后面的元素全排列
                order = allsort(a, order, currentlocal + 1);
    ...
    public ArrayList<ArrayList<Integer>> Finf_Path1(ArrayList<ArrayList<Integer>> all,
                                                           ArrayList<ArrayList<Integer>> path3, int[] Poi_Type) {
        ArrayList<ArrayList<Integer>> path = new ArrayList<>();
...
//        int q){
        ...
        for (int SGq : SG_n) {
            ...
            flag3 = true;
            for (int l = 0; l < SGPoi.getLast().Poi.size(); l++) {//判断这个子图是否有全部的兴趣点
                if (SGPoi.getLast().Poi.get(l).isEmpty()) {
                    flag3 = false;
                    //break;
                    break;
                }
            ...
            if (flag3) {
                ...
                if (min > (int) R) { //网格剪枝
                    ...
                    num_LJ3 = num45 * all.size();
                    ArrayList<ArrayList<Integer>> path8 = Finf_Path1(all1, SGPoi.get(j).Poi, Poi_Type);
                    startTime1 = System.currentTimeMillis();
                    ArrayList<ArrayList<ArrayList<Integer>>> path7 = Find_Path2(path8, Poi_Type, PoiList, all, q, LB, a, endIndex);
                    endTime1 = System.currentTimeMillis();
...
//        System.out.println("第一阶段剪枝消耗的时间为"+time11+",第二阶段剪枝消耗的时间为："+time33);
    ...
    public ArrayList<ArrayList<ArrayList<Integer>>> Find_Path2(ArrayList<ArrayList<Integer>> path, int[] Poi_Type, Poi[] PoiList, ArrayList<ArrayList<Integer>> all,
        ...
        for (ArrayList<Integer> integers : path) {
            ...
            find_Path.getLast().addAll(Finf_Path1(all, path3, Poi_Type));
