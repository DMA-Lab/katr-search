...
ll dijkstra(Poi s, Poi t) {
    ...
    while (!Q.empty()) {
        ...
        if (u.id == t.id) {
            ll ans = ddis[u.id];
            rep(i, 0, cg.size()-1)ddis[cg[i]] = 1e18, Vis[cg[i]] = 0;
            return ans;
        }
...
unordered_map<pair<int, int>,ll, pair_hash> distt;
...
void get_batch_CP_route(CP_set CPSi) {
    ...
    while (!Qr.empty()) {
        ...
        ms[ru] = Gj;
        ru.D = Gj;
        ru.SC = -alpha * Gj + (1 - alpha) * rm;
        SCC.push(-ru.SC); //变成小根堆
...
void get_ans() {
    ...
    while (!Qu.empty()) {
        ...
        vis3[vf.id] = 1; //已遍历可以删
        if (nowQ.t_id.count(vf.key) && !(del[vf.id] || delSG[bel[vf.id]])) {
            //被删的点和子图不考虑作为匹配点
            //与现有关键词匹配
    ...
    sort(all_route.begin(), all_route.end(), [](CP_route a, CP_route b) {
        if (a.SC != b.SC)return a.SC > b.SC;
        else return a.choose_poi[0].id < b.choose_poi[0].id;
    }); //最终找出SC较大的。
...
void di_exploration(query Q) {
    ...
    while (!Qu.empty()) {
        ...
        if (Q.t_id.count(vf.key)) {
            ...
            if (sz >= Q.k) {
                get_k_CP_set(0);
                //cout<<CP_sets[0].choose_poi.size()<<"!!"<<CP_sets[0].choose_poi[0].id<<" "<<CP_sets[0].choose_poi[1].id<<endl;
...
void printroad(Poi s, Poi t) {
    ...
    while (!Q.empty()) {
        ...
        if (u.id == t.id) {
            ...
            while (!st.empty()) {
                if (st.top() != t.id)printf("%d->", st.top());
                st.pop();
            }
            rep(i, 0, cg.size()-1)ddis[cg[i]] = 1e18, Vis[cg[i]] = 0, lst[cg[i]] = 0;
            return;
        }
        rep(i, 0, E[u.id].size()-1) {
            Poi v = _Poi[E[u.id][i].first];
...
int main(int argc, char** argv) {
    ...
    rep(i, 1, 200)insert(i);
    que.vq = _Poi[1];
    auto start = std::chrono::high_resolution_clock::now();
    // 开始时间auto end = std::chrono::high_resolution_clock::now();  // 结束时间
